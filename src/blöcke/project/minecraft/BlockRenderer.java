...
public class BlockRenderer {
    ...
    public BlockRenderer(StaticShader shader, Loader loader){
        this.shader = shader;
        BlockManager.init(loader);
    ...
    public void render(List<BlockChunk> blocks){
        for(BlockChunk block : blocks){
            if(block.getBlockID() != IDManager3.AIR_ID) {
                TexturedModel model = BlockManager.getBlockAt(block.getBlockID());
    ...
    private void prepareInstance(BlockChunk block){
        Matrix4f transformationMatrix = Maths.createTransformationMatrix(block.getPosition(),
                block.getRotation().getX(), block.getRotation().getY(), block.getRotation().getZ(), block.getScale());
